...
import { createSuccessStory, updateSuccessStory, deleteSuccessStory } from "@/actions/admin";
...
export default function SuccessStoriesClient({ stories }: { stories: any[] }) {
  ...
  const [formData, setFormData] = useState({ studentName: "", achievement: "", excerpt: "", content: "", imageUrl: "", slug: "" });
  ...
  const openNew = () => {
    setFormData({ studentName: "", achievement: "", excerpt: "", content: "", imageUrl: "", slug: "" });
    setEditingId(null);
    setShowForm(true);
  };
  ...
  const handleSaveCrop = async () => {
    ...
    try {
      ...
      if (croppedBlob) {
        ...
        formDataPayload.append("file", croppedBlob, "success-story.jpg");
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-bold text-gray-800">Quản lý Câu chuyện thành công</h2>
      ...
      {showForm && (
        <div className="mb-8 bg-white p-6 rounded-xl shadow-sm border border-gray-100">
          <h3 className="text-xl font-bold mb-4 text-gray-800">{editingId ? "Sửa câu chuyện" : "Thêm câu chuyện mới"}</h3>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Tên học sinh</label>
                <input required type="text" name="studentName" value={formData.studentName} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-green-500 focus:border-green-500" />
              </div>
              <div>
              ...
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Thành tích nổi bật</label>
                <input required type="text" name="achievement" value={formData.achievement} onChange={handleChange} className="w-full px-3 py-2 border rounded-lg focus:ring-green-500 focus:border-green-500" placeholder="VD: Thủ khoa ĐH Bách Khoa" />
              </div>
              
              {/* Image Upload Area */}
              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Ảnh học viên</label>
                <div className="flex items-start gap-4">
                  {formData.imageUrl ? (
                    <div className="relative w-32 h-20 rounded-lg overflow-hidden border border-gray-200 shadow-sm">
                      <img src={formData.imageUrl} alt="Preview" className="w-full h-full object-cover" />
                      <button type="button" onClick={() => setFormData({...formData, imageUrl: ''})} className="absolute top-1 right-1 bg-white/80 p-1 rounded-full text-red-500 hover:bg-white hover:text-red-700">
                        <X size={14} />
                      </button>
                    </div>
                  ) : (
                  ...
                  <div className="flex-1">
                    ...
                    <p className="mt-2 text-xs text-gray-500">Kéo thả để cắt ảnh trực tiếp trên trình duyệt.</p>
                  </div>
                </div>
              </div>

              <div className="md:col-span-2">
                <label className="block text-sm font-medium text-gray-700 mb-1">Đoạn trích (Hiển thị ở trang ngoài)</label>
                <textarea name="excerpt" value={formData.excerpt} onChange={handleChange} rows={2} className="w-full px-3 py-2 border rounded-lg focus:ring-green-500 focus:border-green-500"></textarea>
              </div>
              <div className="md:col-span-2">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="bg-white rounded-xl w-full max-w-lg overflow-hidden flex flex-col h-[500px]">
            <div className="p-4 border-b border-gray-200 flex justify-between items-center">
              <h3 className="font-bold text-lg">Cắt ảnh học viên</h3>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="py-3 px-4 text-sm font-semibold text-gray-600">Thành tích</th>
            ...
            <tbody>
              {stories.map((story) => (
                <tr key={story.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="py-3 px-4 text-sm text-gray-500">#{story.id}</td>
                  <td className="py-3 px-4">
                    {story.imageUrl ? (
                      <img src={story.imageUrl} alt={story.studentName} className="w-16 h-9 rounded-md object-cover border border-gray-200" />
                    ) : (
                      <div className="w-16 h-9 rounded-md bg-gray-100 flex items-center justify-center text-gray-400 text-[10px]">Trống</div>
                    )}
                  </td>
                  <td className="py-3 px-4 text-sm text-gray-800 font-medium">{story.studentName}</td>
                  <td className="py-3 px-4 text-sm text-gray-600">{story.achievement}</td>
                  <td className="py-3 px-4 text-right">
                    <button onClick={() => openEdit(story)} className="text-green-600 hover:text-green-800 p-1 mx-1">
                      <Edit size={18} />
                    </button>
                    <button onClick={() => handleDelete(story.id)} className="text-orange-500 hover:text-orange-600 p-1 mx-1">
                      <Trash2 size={18} />
                    </button>
                  </td>
                </tr>
              ...
              {stories.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-6 text-center text-gray-500">Chưa có dữ liệu.</td>
                </tr>
              )}
            </tbody>
          </table>
