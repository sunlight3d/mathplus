...
def mcases(*rows):
    # Curly brace system of equations
    arr = meqarr(*rows)
    return f'<m:d><m:dPr><m:begChr m:val="&#x007B;"/><m:endChr m:val=""/></m:dPr><m:e>{arr}</m:e></m:d>'
...
def build_mathplus_exam_docx(output_path):
    ...
    # Page setup - A4, margins 1.6cm top/bottom, 2.0cm left/right
    ...
    # ----------------------------------------------------
    # HEADER CONFIGURATION (Logo on Left, Center Title)
    # ----------------------------------------------------
    header = section.header
    header.is_linked_to_previous = False
    
    htable = header.add_table(rows=1, cols=3, width=Cm(17.0))
    htable.alignment = WD_TABLE_ALIGNMENT.CENTER
    htable.autofit = False
    htable.columns[0].width = Cm(3.0)
    htable.columns[1].width = Cm(11.0)
    htable.columns[2].width = Cm(3.0)
    ...
    if os.path.exists(logo_path):
        ...
        run_img.add_picture(logo_path, height=Cm(1.15))
    ...
    run_htext.font.color.rgb = RGBColor(27, 94, 32)
    
    # Right cell: balance
    c2 = htable.cell(0, 2)
    c2.width = Cm(3.0)
    c2.vertical_alignment = WD_ALIGN_VERTICAL.CENTER
    p_r_dummy = c2.paragraphs[0]
    p_r_dummy.paragraph_format.space_after = Pt(0)
    p_r_dummy.paragraph_format.space_before = Pt(0)
    ...
    run_foot = fp.add_run("GV : Nguyễn Dũng - 0977961189")
    run_foot.bold = True
    ...
    set_cell_margins(box_cell, top=30, bottom=30, left=60, right=60)
    ...
    sys_eq = mcases(mr('ax + by = 3'), mr('2ax - 3by = 1'))
    p_b3_3._p.append(create_math_element(sys_eq))
    p_b3_3.add_run(" có nghiệm ")
    p_b3_3._p.append(create_math_element(mr('(x; y) = (2; 1)')))
    p_b3_3.add_run(".")
    ...
    p_b4_2_a.add_run("AB = 3 cm; BC = 5 cm.")
    ...
    p_b4_2_c.add_run(" (")
    p_b4_2_c._p.append(create_math_element(mr('I')))
    p_b4_2_c.add_run(" thuộc ")
    p_b4_2_c._p.append(create_math_element(mr('DE')))
    p_b4_2_c.add_run("), ")
